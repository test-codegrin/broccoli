...
export async function GET(_request, { params }) {
  const blogDetailId = Number.parseInt(params.id, 10);
  if (!Number.isFinite(blogDetailId) || blogDetailId <= 0) {
  ...
  return NextResponse.json({
    data: {
      ...data,
      excerpt: stripHtml(data.blog_description || "").slice(0, 190),
      content_sequence: buildContentSequence(data),
    },
  });
}
